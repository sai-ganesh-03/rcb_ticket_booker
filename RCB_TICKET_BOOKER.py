...
class RCBTicketBooker:
    ...
    def get_stands_list(self, event_code,mobile,auth_token):
        url = f"https://rcbmpapi.ticketgenie.in/ticket/standslist/{event_code}"
    ...
    def book_tickets(self,event_data, mobile, auth_token):
        ...
        # Get stands list
        stands_data = self.get_stands_list(event_code, mobile, auth_token)
        ...
        lowest_priced_stands = sorted_stands[:3]  # Only considering the two lowest priced stands
        ...
        if len(lowest_priced_stands) == 0:
            logger.info(f"{mobile} - Stands with lowest 3 price are not present")
            send_telegram_messages(f"{mobile} - Stands with lowest 3 price are not present")
            return
        
        for stand in lowest_priced_stands:
            stand_code = stand['stand_Code']
            logger.info(f"{mobile} - Checking stand {stand_code} with price {stand.get('price')}.")
